...
export class SessionSyncStore {
  ...
  commitBatch(input: CommitSessionBatchInput): CommitSessionBatchResult {
    ...
    if (!Number.isSafeInteger(input.sequence) || input.sequence < 0) {
      throw new Error("Sync batch sequence must be a non-negative safe integer.");
    }
    ...
    for (const item of input.items) {
      ...
      if (sourceIds.has(item.sourceId)) {
        throw new Error(`Duplicate session source ID in batch: ${item.sourceId}`);
      }
    ...
    return this.db.transaction(() => {
      ...
      if (!state || state.activeRunId !== input.runId || state.activeMode === null) {
