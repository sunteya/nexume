...
describe("session storage", () => {
  test("isolates identical source IDs by collector and agent", async () => {
    ...
    expect(storage.sessions.list({ limit: 10 }).items.map((row) => [
      row.collectorId,
      row.agent,
      row.sourceId,
    ])).toEqual([
  ...
  test("upserts sessions and makes run sequence retries idempotent", async () => {
    ...
    expect(storage.sessions.get({
      collectorId: "collector-a",
      agent: "opencode",
      sourceId: "one",
    })).toEqual(expect.objectContaining({ title: "Updated", sourceUpdatedAt: 20 }));
  ...
  test("does not delete on interrupted reconcile and deletes only on completion", async () => {
    ...
    expect(storage.sessions.list({ limit: 10 }).items.map((row) => row.sourceId)).toEqual([
      "keep",
      "missing",
    ]);
    ...
    expect(storage.sessions.list({ status: "deleted", limit: 10 }).items).toEqual([
      expect.objectContaining({ sourceId: "missing", deletedAt: expect.any(Number) }),
    ]);
    storage.close();
  });
  ...
  test("restores a deleted session when it appears again", async () => {
    ...
    expect(storage.sessions.list({ status: "deleted", limit: 10 }).items).toHaveLength(1);
  ...
  test("filters statuses and scopes while paginating with structured cursors", async () => {
    ...
    expect(storage.sessions.list({
      collectorId: "collector-a",
      agent: "opencode",
      limit: 10,
    }).items).toHaveLength(3);
    expect(storage.sessions.list({ status: "archived", limit: 10 }).items).toEqual([
      expect.objectContaining({ sourceId: "archived" }),
    ]);
    storage.close();
  });
  ...
  test("persists sessions, checkpoints, active runs, and batch idempotency", async () => {
    ...
    expect(second.sessionSync.commitBatch({
      collectorId: "collector-a",
      agent: "opencode",
      runId: "persistent",
      sequence: 0,
