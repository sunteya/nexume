...
桌面端默认采集本机 Claude Code、Codex、OpenCode 和 Alma Session。Codex 包括更名后的 ChatGPT 桌面应用中的 Codex 历史，优先读取有效的 `$CODEX_HOME/state_5.sqlite`，否则读取 `~/.codex/state_5.sqlite`；普通 ChatGPT 聊天历史不在采集范围内。Claude Code 默认读取 `${CLAUDE_CONFIG_DIR:-~/.claude}/projects`，只将主会话作为独立 Session 采集。可通过 `CLAUDE_PROJECTS_PATH`、`CODEX_DB_PATH`、`OPENCODE_DB_PATH` 和 `ALMA_DB_PATH` 覆盖数据路径。
